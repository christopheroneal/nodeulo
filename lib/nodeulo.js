...
function Accumulo(options) {
  ...
  this.scan = function(options, callback) {
    ...
    var scanOptions = new accumuloTypes.ScanOptions();
  ...
  this.batchScan = function(options, callback){
    options = merge({
      ranges: null,
      columns: null,
      authorizations: null,
    ...
    var batchScanOptions = new accumuloTypes.BatchScanOptions();
    batchScanOptions.ranges = options.ranges ? options.ranges.map(function(range){
      return getRange(range);
    }) : null;
    batchScanOptions.columns = getScanColumns(options.columns);
    batchScanOptions.iterators = getIteratorSettings(options.iterators);
    proxy.createBatchScanner(login, options.table, batchScanOptions, function(err, scanner) {
      performScan(scanner, options.batchSize, function(results) {
        callback(null, results);
      });
    });
