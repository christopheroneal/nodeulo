...
Range.prototype.out() = function() {
  ...
  range.startInclusive = start.include;
  range.endInclusive = end.include;
  ...
  if (this.end.row) {
    range.end = new accumuloTypes.Key();
    range.end.row = this.end.row;
    range.end.colFamily = this.end.columnFamily;
    range.end.colQualifier = this.end.columnQualifier;
    range.end.timestamp = this.end.timestamp;
    range.end.colVisibility = this.end.columnVisibility;
  }
  return range;
};
